...
layerstenplusurl = \
    ['http://ags.gis.iastate.edu/arcgisserver/services/Ortho/',
...
def serverError(
    ...
    i,
    url,
    ): # where we're going, we dont need errors. if needed, add in a messagebox with the following stuff, or similar
    print 'Server returned error ' + str(jpg.status_code) \
        + ' with reason "' + str(jpg.reason) + '" for file "' + i + '".'
    print 'URL: ' + jpg.url
    print ''
...
-''' + str(zoom) + '\n' + str(x)
    ...
    if i == 'S2014C':  # workaround until we see if 2015 has the same server
        jpg = \
            requests.get('http://gis.apfo.usda.gov/arcgis/services/NAIP/Iowa_2014_1m_NC/ImageServer/WMSServer?layers='
                          + str(layerstenpluscodes.get(i)),
                         params=layersdata)
        print 'Using workaround for ' + str(jpg.url)
        if int(jpg.status_code) == 200:
            f = open(str(self.browseEdit.currentText()) + i + '.jpg',
                     'wb', 0)
            f.write(jpg.content)
            f.close()
    else:
        ...
        curval = layerstenminuscodes.get(i)
        layersdata['layers'] = curval
        jpg = requests.get(layerstenplusurl[0]
                           + str(layerstenpluscodes.get(i))
                           + layerstenplusurl[1], params=layersdata)
        if int(jpg.status_code) == 200:
            f = open(str(self.browseEdit.currentText()) + i + '.jpg',
                     'wb', 0)
            f.write(jpg.content)
